...
2. **Drain in `service()`:** the main loop calls `AudioEngine::service()`, which pops jobs and routes them to `handlePreload()` or `handleFade()`.
...
- Foreground loop hook: `AudioEngine::service()` in `firmware/arduino/lofi_sampler/AudioEngine.cpp` (called from `loop()` in `lofi_sampler.ino`).
- Job producers: `preloadAndPlay()`, `setLevel()`, `stopVoice()`, `requestDiagnostics()` in `AudioEngine.h`/`.cpp`—these enqueue `Job` structs consumed by `service()`.
- Job consumers and buffer plumbers: `handlePreload()`, `handleFade()`, `handleDiagnostics()`, plus `pumpStreams()`/`pumpGains()` in `AudioEngine.cpp`.
